<script>
  ...
  const updateFromPeer = async (peer) => {
    const ok = await askConfirm(
      `${peer.name} is running a newer SideSave build (${peer.appVersion}). Download it from ${peer.name} and update this device? SideSave restarts itself when done.`,
      { title: 'Update SideSave?', confirmText: 'Update & restart' }
    );
    if (!ok) return;
      ...
      <div class="card peer">
        ...
        <div class="peer-info">
          ...
          <div class="peer-meta">
            ...
            {#if peer.appVersion}· SideSave {peer.appVersion}{/if}
          </div>
        </div>
...
{#if connectTab === 'lan'}
  <p class="quiet lan-intro">
    Devices running SideSave on the same Wi-Fi/Ethernet discover each other automatically. No setup needed.
  </p>
  <h4 class="subsection">Found on your network</h4>
  {#if lanDiscovered.length === 0}
    <p class="quiet">No unpaired devices found on the local network. Make sure SideSave is running on the other device.</p>
  {:else}
    <div class="list">

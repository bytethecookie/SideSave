<script>
  ...
  async function startAuth() {
    ...
    try {
      ...
      if (authAuto) {
        toast('Finish signing in — SideSave connects automatically.');
      } else {
  ...
  const runExport = async () => {
    ...
    const target = await native.selectSaveFile('Export selected saves', 'sidesave-saves.sscb');
    if (!target) return;
    exporting = true;
    try {
  ...
  <div class="card">
    ...
    {#if config.provider === 'local'}
      <div class="field">
        ...
        <div class="path-row">
          <input id="cb-folder" bind:value={config.url} placeholder="D:\Backups\SideSave" />
    ...
    {:else if config.provider === 'webdav'}
      <div class="field">
        ...
        <input id="cb-url" bind:value={config.url} placeholder="https://nas.local/dav/sidesave/" />
      </div>
      <div class="two">
        <div class="field">
        ...
        {#if !authInProgress}
          ...
          {#if config.provider === 'google_drive'}
            <p class="quiet" style="margin-top: 10px;">
              ⚠️ On Google's consent screen, <strong>tick the checkbox</strong> allowing SideSave to access
        ...
        {:else}
          <div class="auth-waiting">
            ...
            <div class="auth-waiting-text">
              ...
              <span class="quiet">Approve access in your browser — SideSave connects by itself.</span>
            </div>

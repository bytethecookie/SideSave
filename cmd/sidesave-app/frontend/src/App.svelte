...
<div class="shell">
    ...
    <div class="update-banner installing">
      <span>
        {#if $appUpdate.state === 'downloading'}
          ⬇️ Updating SideSave — downloading {$appUpdate.percentage ?? 0}%…
  ...
  {:else if update}
    <div class="update-banner">
      <span>🎉 SideSave {update.latest} is available — you're on {update.current}.</span>
  ...
  {:else if updatedTo}
    <div class="update-banner">
      <span>🎉 SideSave was updated to v{updatedTo}.</span>
      <div class="update-actions">
  ...
  <div class="body">
    {#if bootError}
      <div class="boot-error">
        <img class="boot-logo" src={logoUrl} alt="SideSave" />
        <h2>SideSave failed to start</h2>
        <p>{bootError}</p>
        <button class="btn primary" disabled={retrying} on:click={boot}>
          {retrying ? 'Retrying…' : 'Retry'}
        </button>
        <p class="boot-hint">
          Details are saved to <code>.sidesave\sidesave.log</code> in your user folder.
        </p>
      </div>
    {:else if ready}
      <Sidebar />
    ...
    {:else}
      <div class="boot-loading">
        <img class="boot-logo pulse" src={logoUrl} alt="SideSave" />
        <span>Starting SideSave…</span>
      </div>
    {/if}
  </div>
  <StatusBar />

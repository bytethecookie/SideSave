<script>
  ...
  onMount(async () => {
    try {
      ...
      info = { name: 'SideSave', version: '2.0.0' };
    }
  });
...
<div class="backdrop" use:backdropClose={onClose} role="presentation">
  <div class="modal" role="dialog" aria-modal="true" aria-label="About SideSave">
    <button class="x" on:click={onClose} title="Close" aria-label="Close">✕</button>
    <img class="logo" src={logoUrl} alt="" />
    <h2>{info?.name ?? 'SideSave'}</h2>
    <div class="ver">
    ...
    <p class="note">Wire-compatible with the original Node.js/Electron SideSave — Go and JS devices sync together.</p>
  </div>
</div>

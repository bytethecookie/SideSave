// Command sidesave-app is the SideSave desktop application: the daemon
...
func main() {
	...
	err := wails.Run(&options.App{
		...
		Frameless:        true,
		// Only one SideSave window ever; a second launch focuses the existing
		// one instead of opening a duplicate (blank) window.
		SingleInstanceLock: &options.SingleInstanceLock{
			UniqueId:               "sidesave-desktop-single-instance",
		...
		Linux: &linux.Options{
			...
			ProgramName:         "SideSave",
			WindowIsTranslucent: false,
			WebviewGpuPolicy:    linux.WebviewGpuPolicyOnDemand,
		},
	})
	if err != nil {
		fmt.Fprintf(os.Stderr, "sidesave: %v\n", err)
		os.Exit(1)
	}
}

...
func TestSelectUpdateAssetFor(t *testing.T) {
	assets := []releaseAsset{
		{Name: "SideSave.Setup.exe", BrowserDownloadURL: "u/setup"},
		{Name: "SideSave.exe", BrowserDownloadURL: "u/portable"},
		{Name: "sidesave-cli.exe", BrowserDownloadURL: "u/cli"},
		{Name: "sidesave-relay.exe", BrowserDownloadURL: "u/relay"},
		{Name: "sidesave-linux-amd64.tar.gz", BrowserDownloadURL: "u/linux"},
	}
...
func TestExtractAppBinary(t *testing.T) {
	// Build a tarball like the release: sidesave-linux/{sidesave,cli,relay}.
	var buf bytes.Buffer
	gz := gzip.NewWriter(&buf)
	tw := tar.NewWriter(gz)
	files := map[string]string{
		"sidesave-linux/sidesave-cli":   "cli-bytes",
		"sidesave-linux/sidesave":       "APP-BINARY-CONTENT",
		"sidesave-linux/sidesave-relay": "relay-bytes",
	}
	...
	dest := filepath.Join(t.TempDir(), "sidesave.new")
	if err := selfupdate.ExtractFromTarGz(archive, "sidesave", dest); err != nil {

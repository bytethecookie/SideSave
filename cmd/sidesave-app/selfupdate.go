...
const flatpakUpdateMsg = "SideSave is installed as a Flatpak, which updates through Flatpak itself — " +
	"run \"flatpak update\", or install the newer SideSave.flatpak from the GitHub release page."
	...
	go func() {
		...
		if !selfupdate.CanStageUpdate(exe) {
			a.updateEvent("error", 0,
				"SideSave is installed in a protected folder (like Program Files), which peer updates can't replace. "+
		...
		a.daemon.Log.Log("info", fmt.Sprintf("downloaded SideSave %s (build %d) from peer; installing", build.AppVersion, build.BuildTimeMs))
...
func (a *App) InstallUpdateFromURL(url string) string {
	...
	go func() {
		...
		if strings.HasSuffix(strings.ToLower(url), ".tar.gz") || strings.HasSuffix(strings.ToLower(url), ".tgz") {
			...
			if err := selfupdate.ExtractFromTarGz(archivePath, "sidesave", newBinary); err != nil {
...
func (a *App) installViaInstaller() {
	...
	tmp, err := os.CreateTemp("", "SideSave.Setup-*.exe")
	if err != nil {
		a.updateEvent("error", 0, err.Error())
		return
	}
...
func (a *App) fetchInstallerURL() (string, error) {
	rel, err := selfupdate.LatestRelease(updateRepo, "SideSave/"+AppVersion, a.wantsPreReleases())
	if err != nil {
		return "", err
	}
...
func (a *App) applyUpdate(newExePath string) error {
	...
	cmd.Env = append(os.Environ(), "SIDESAVE_CLEANUP_OLD="+old)
	if err := cmd.Start(); err != nil {
		_ = os.Remove(exe)
		_ = os.Rename(old, exe)
...
func cleanupReplacedBinary() {
	if old := os.Getenv("SIDESAVE_CLEANUP_OLD"); old != "" {

...
const AUTO_SYNC_KEY = "sidesave:autoSyncOnGameEvents";
...
function Content() {
  ...
  useEffect(() => {
    ...
    addEventListener<[string, any]>("sidesave_sync", onSync);
    return () => removeEventListener<[string, any]>("sidesave_sync", onSync);
  }, []);
  ...
  const onStartDaemon = async () => {
    ...
    try {
      ...
      if (res.success) {
        toaster.toast({ title: "SideSave", body: "Sync service started" });
      } else {
        toaster.toast({ title: "SideSave", body: res.error ?? "Couldn't start the sync service" });
      }
      await refresh();
    } finally {
      setStarting(false);
...
function registerLifecycleHooks(): () => void {
  ...
  if (!client?.GameSessions?.RegisterForAppLifetimeNotifications) {
    console.warn("[SideSave] SteamClient.GameSessions unavailable; auto-sync disabled");
    return () => {};
  }
  ...
  const syncFor = async (appId: string, when: "launch" | "exit") => {
    ...
    try {
      ...
      console.error("[SideSave] lifecycle sync failed", e);
    } finally {
...
export default definePlugin(() => {
  ...
  return {
    name: "SideSave",
    title: <div className={staticClasses.Title}>SideSave</div>,
    content: <Content />,
    icon: <FaSave />,
    onDismount() {
      disposeHooks();

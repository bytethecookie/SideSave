...
Findings from an audit of `sidesave-decky-plugin/` and the Flatpak packaging,
...
`packaging/flatpak/io.github.bytethecookie.SideSave.yml`):

```
install -Dm755 sidesave-cli /app/bin/sidesave-cli
```

so the backend can start a daemon itself:

```bash
flatpak run --command=sidesave-cli io.github.bytethecookie.SideSave daemon
```

### Tasks
- [x] Add a `start_daemon` backend method in `main.py` that spawns the above
      detached, and returns whether it came up.
- [x] Surface a **Start SideSave** button in the panel whenever the daemon is
...
- [x] Decide the lifetime model and document it. Both are now supported:
      **on-demand** (the panel's *Start sync service* button) for a zero-setup
      path, and **persistent** via `packaging/systemd/sidesave-daemon.service`
...
is exactly the manual step SideSave exists to remove.

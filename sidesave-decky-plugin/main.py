"""SideSave Decky Loader backend.

Bridges the Game Mode panel to the SideSave daemon's local HTTP API, and can
...
ADDR_FILE = os.path.join(decky.DECKY_USER_HOME, ".sidesave", "daemon.addr")
FALLBACK_URL = "http://127.0.0.1:8383"

FLATPAK_APP_ID = "io.github.bytethecookie.SideSave"
...
class Plugin:
    ...
    async def start_daemon(self) -> dict:
        ...
        bring the daemon up itself. Prefers a native sidesave-cli on PATH and
        ...
        if cmd is None:
            return {
                "success": False,
                "error": "SideSave isn't installed on this device — install the Flatpak from Desktop Mode first.",
            }
    ...
    def _daemon_command(self):
        ...
        native = shutil.which("sidesave-cli")
        if native:
            return [native, "daemon"]
        flatpak = shutil.which("flatpak")
        if flatpak:
            return [flatpak, "run", "--command=sidesave-cli", FLATPAK_APP_ID, "daemon"]
        return None
    ...
    async def _main(self):
        ...
        decky.logger.info("SideSave plugin loaded")
    ...
    async def _unload(self):
        ...
        if task:
            task.cancel()
        decky.logger.info("SideSave plugin unloaded")

    async def _uninstall(self):
        decky.logger.info("SideSave plugin uninstalled")
